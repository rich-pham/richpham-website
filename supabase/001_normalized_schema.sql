-- ============================================================
-- Rich Pham: Normalized CRM Schema
-- Migration: 001_normalized_schema
-- Run this in Supabase SQL Editor (https://supabase.com/dashboard)
--
-- Adapted from the AI Officer Company schema. Rich does not run
-- retreats — inquiry types are limited to:
--   general | coaching | speaker | newsletter
--
-- Tables:
--   people        — one row per unique email, shared across forms
--   inquiries     — typed form submissions linked to people
--   activity_log  — CRM action audit trail
-- ============================================================
...
create table if not exists public.people (
  ...
  company        text,
  role           text,
  source_site    text default 'richpham.com',
...
create table if not exists public.inquiries (
  ...
  type            text not null default 'general'
                    check (type in ('general', 'coaching', 'speaker', 'newsletter')),
  subject         text,
  message         text,
...
begin
  ...
  insert into public.inquiries (person_id, type, subject, message, metadata, source, source_site)
  values (v_person_id, p_type, p_subject, p_message, coalesce(p_metadata, '{}'::jsonb), p_source, p_source_site)
...
begin
  return query
  select
    i.id, i.type, i.status, i.subject, i.message, i.metadata,
    i.source, i.source_site, i.created_at,
